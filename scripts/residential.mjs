 ...
 * or two courts is, in practice, a court in someone's yard. Listing it sends
 ...
 * to backfill already-published files:
...
export function isResidentialGuess(f) {
  const unnamed = f.name === FALLBACK_NAME || f.name.includes(" · ");
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  for (const file of readdirSync(dir).filter((f) => /^courts-\w+\.json$/.test(f))) {
    ...
    for (const f of data.facilities) if (f.tags?.context === "residential") delete f.tags.context; // idempotent
